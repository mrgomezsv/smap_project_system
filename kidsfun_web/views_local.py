...
# from api_like.models import Like  # Comentado
# from api_commentary.models import Commentary  # Comentado
...
def web_comment(request, product_id):
    ...
    try:
        # Obtener datos del request
        data = json.loads(request.body)
        user_name = data.get('user_name', 'Usuario Web')
        comment_text = data.get('comment', '')
        
        # Comentado: Crear el comentario en la base de datos
        ...
        return JsonResponse({
            'success': True,
            'message': 'Comentario agregado exitosamente (modo local)',
...
def custom_404(request, exception):
    return render(request, 'kidsfun_web/404.html', status=404) 